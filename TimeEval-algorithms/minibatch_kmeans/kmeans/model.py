...
class KMeansAD(BaseEstimator, OutlierMixin):
    def __init__(self, k: int, window_size: int, stride: int, batch_size: int):
        self.k = k
        self.window_size = window_size
        self.stride = stride
        self.model = MiniBatchKMeans(n_clusters=k, batch_size=batch_size)
        self.padding_length = 0

    def _preprocess_data(self, X: np.ndarray) -> np.ndarray:
        flat_shape = (X.shape[0] - (self.window_size - 1), -1)  # in case we have a multivariate TS
        slides = sliding_window_view(X.astype(np.float32), window_shape=self.window_size, axis=0).reshape(flat_shape)[::self.stride, :]

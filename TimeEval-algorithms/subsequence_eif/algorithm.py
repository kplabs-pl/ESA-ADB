...
class Config:
    ...
    def __init__(self, params):
        ...
        self.window_size = customParameters.get("window_size", 100)
...
def train_n_execute(config: Config):
    ...
    X = sliding_window_view(X, window_shape=config.window_size, axis=0).reshape(-1, config.window_size * X.shape[1])
    ...
    scores = np.pad(scores, config.window_size // 2, constant_values=0)

...
class Errors:
    def __init__(self, channel, config, run_id, save: bool = True):
        ...
        y_test = channel.test[0, self.config.window_size:-self.config.prediction_window_size, channel.generator_test.target_channel_index]
        self.e = np.abs(channel.y_hat - y_test)

        smoothing_window = int(self.config.batch_size * self.config.smoothing_window_size
                               * self.config.smoothing_perc)
        if not len(channel.y_hat) == len(y_test):
            raise ValueError('len(y_hat) != len(y_test): {}, {}'
                             .format(len(channel.y_hat), len(y_test)))

...
def execute(args: AlgorithmArgs):
    data = args.ts
    detector = load(args)
    anomaly_scores, reconstruction = detector.detect(data)
    np.savetxt(args.modelOutput + ".reconstruction.csv", reconstruction, delimiter=",", header=",".join(args.df.columns[1:-1]), comments="")

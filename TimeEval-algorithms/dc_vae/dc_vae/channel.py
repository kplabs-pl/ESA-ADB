...
import os
import logging
from .generators import DcVaeGenerator
...
class Channel:
    def __init__(self, config):
        ...
        self.nb_input_channels = None
        self.nb_output_channels = None
        self.y_hat = None
    ...
    def shape_data(self, arr, binary_channels_mask, channels_minimums, channels_maximums, train=True):
        ...
        if train:
            if isinstance(arr, list):
                ...
                nb_train_samples = round((1 - self.config.val_percent) * total_nb_samples)
            ...
            self.generator_train = DcVaeGenerator(train_data, train_means, train_stds, self.config.input_channel_indices, self.config.target_channel_indices, self.config.window_size,
                                                  self.config.batch_size, shuffle=True)
            self.generator_val = DcVaeGenerator(val_data, train_means, train_stds, self.config.input_channel_indices, self.config.target_channel_indices, self.config.window_size,
            ...
            self.generator_test = DcVaeGenerator(arr, train_means, train_stds, self.config.input_channel_indices, self.config.target_channel_indices, self.config.window_size,
                                                 self.config.batch_size, shuffle=False, prediction_mode=True)

        self.nb_input_channels = len(self.config.input_channel_indices)
        self.nb_output_channels = len(self.config.target_channel_indices)

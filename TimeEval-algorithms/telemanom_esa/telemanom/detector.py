...
class Detector:
    ...
    def predict(self, channels: Channel, args, thresholded: bool = False) -> np.ndarray:
        model = Model(self.config, self.id, channels, model_path=self.model_path)
        model.batch_predict(channels, save_y_hat=True)
        errors = Errors(channels, model, self.config, self.id, save=False)

        padded_errors = np.concatenate((np.zeros((self.config.window_size, errors.e_s.shape[-1])), errors.e_s))

        if thresholded:
            thresholded_scores = np.zeros_like(model.y_hat)

            del model  # the model data is no longer needed
            gc.collect()

            for channel_idx in self.config.target_channel_indices:
                errors.process_batches(channels, channel_idx)
                for start_idx, end_idx in errors.E_seq:
                    thresholded_scores[start_idx:end_idx, self.config.target_channel_indices.index(channel_idx)] = 1

            thresholded_scores = np.concatenate(
                (np.zeros((self.config.window_size, thresholded_scores.shape[-1])), thresholded_scores))

            np.savetxt(args.dataOutput.replace(".csv", "-no-threshold.csv"), padded_errors, delimiter=",")
        ...
        for i, row in self.chan_df.iterrows():
            ...
            if with_threshold:
                ...
                result_row = {
                    'run_id': self.id,
                    'chan_id': row.chan_id,
                    'num_train_values': len(channel.generator_train.nb_samples),
                    'num_test_values': len(channel.generator_val.nb_samples),

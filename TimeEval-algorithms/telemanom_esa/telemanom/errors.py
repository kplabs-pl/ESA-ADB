...
class Errors:
    def __init__(self, channel, model, config, run_id, save: bool = True):
        """
        ...
        self.n_windows = len(channel.generator_test)

        # raw prediction error
        channel_data = channel.generator_test.data[0, self.config.window_size:][..., self.config.target_channel_indices]

        if not len(model.y_hat) == len(channel_data):
            raise ValueError('len(y_hat) != len(y_test): {}, {}'
                             .format(len(model.y_hat), len(channel_data)))
        normalization_ptp = np.ptp(channel_data, axis=0)

        e = np.abs(model.y_hat - channel_data)
        ...
        self.e_s = pd.DataFrame(e).ewm(span=smoothing_window, axis=0).mean().values
        ...
        if not channel.id == 'C-2':  # anomaly occurs early in window
            self.e_s[:self.config.window_size] = \
                np.array([np.mean(self.e_s[:self.config.window_size * 2], axis=0)] * self.config.window_size)

        if save:
            self.save_errors(run_id, channel.id)

        self.normalized = np.mean(e / normalization_ptp, axis=0)
        logger.info(f"normalized prediction error: {self.normalized}")
    ...
    def adjust_window_size(self, channel):
        ...
        while self.n_windows < 0:
            self.window_size -= 1
            self.n_windows = int((channel.generator_test.data.shape[1]
            ...
            if self.window_size == 1 and self.n_windows < 0:
                raise ValueError('Batch_size ({}) larger than y_test (len={}). '
                                 ...
                                 .format(self.config.batch_size,
                                         channel.generator_test.data.shape[1]))


    def process_batches(self, channels, channel_idx):
        """
        Top-level function for the Error class that loops through batches
        of values for a channel.

        Args:
            channels (obj): Channel class object containing train/test data
                for X,y for a single channel
        """
        ...
        self.adjust_window_size(channels)

        for i in range(0, self.n_windows):
            prior_idx = i * self.config.batch_size
            idx = (self.config.smoothing_window_size * self.config.batch_size) + (i * self.config.batch_size)
            if i == self.n_windows-1:
                idx = channels.generator_test.data.shape[1]

            window = ErrorWindow(channels, self.config, prior_idx, idx, self, i, channel_idx)
...
class ErrorWindow:
    def __init__(self, channel, config, start_idx, end_idx, errors, window_num, channel_idx):
        """
        ...
        Args:
            ...
            channel_idx (int): Channel index to calculate errors for in multioutput setting
        ...
        self.e_s = errors.e_s[start_idx:end_idx, self.config.target_channel_indices.index(channel_idx)]
        ...
        self.y_test = channel.generator_test.data[0, start_idx:end_idx, channel_idx]
        ...
        if channel.generator_test.data.shape[1] < 2500:
            self.num_to_ignore = self.config.window_size
        if channel.generator_test.data.shape[1] < 1800:
    ...
    def compare_to_epsilon(self, errors_all, inverse=False):
        ...
        if not (self.sd_e_s > (.05 * self.sd_values) or max(self.e_s) > (.05 * self.inter_range)) \
                or len(e_s) < self.config.error_buffer \
                or not max(self.e_s) > self.config.min_error_value:
            return

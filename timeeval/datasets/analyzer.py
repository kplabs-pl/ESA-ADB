...
class DatasetAnalyzer:
    ...
    def __init__(self, dataset_id: DatasetId, is_train: bool,
        ...
        if ignore_stationarity:
            self.stationarity = dict(
                ...
                for _, series in self._df.iloc[:, 1:-self.dimensions].items()
            )
        else:
    ...
    def metadata(self) -> DatasetMetadata:
        ...
        return DatasetMetadata(
            ...
            contamination=self.contaminations,
            means=self.means,
    ...
    def _find_base_metadata(self) -> None:
        ...
        self.dimensions = (len(self._df.columns) - 1) // 2

        contaminations = self._df.iloc[:, -self.dimensions:].sum(axis=0) / self.length
        contaminations = dict(contaminations)
        contaminations = {key[len("is_anomaly_"):]: value for key, value in contaminations.items()}
        self.contaminations = contaminations

        means = self._df.iloc[:, 1:-self.dimensions].mean(axis=0)
        stddevs = self._df.iloc[:, 1:-self.dimensions].std(axis=0)
        self.means = dict(means.items())
        self.stddevs = dict(stddevs.items())

        self.num_anomalies = dict()
        self.anomaly_length = dict()

        for channel_name in self.contaminations.keys():
            labels = self._df[f"is_anomaly_{channel_name}"] if f"is_anomaly_{channel_name}" in self._df else self._df[f"is_anomaly"]
            ...
            self.num_anomalies[channel_name] = len(anomalies)
            self.anomaly_length[channel_name] = AnomalyLength(
    ...
    def _find_stationarity(self) -> None:
        ...
        df = df.set_index("timestamp").iloc[:, :-self.dimensions]
        self.stationarity = {}
